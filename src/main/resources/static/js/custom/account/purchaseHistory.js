...
$(function () {
    dataTable = $('#payments').DataTable({
        ...
        "scrollCollapse": true,
        'ajax': {
            'url': context + 'purchasedPlans/get.json',
        ...
        'createdRow': function (row, data, index) {
            ...
            var purchaseLink = '<a class="text-center" href="' + context + 'purchasedPlans/purchase/MyBooking/' + purchaseId + ' "> \
                    <span class="glyphicon glyphicon-chevron-right"></span></a>';
            $('td', row).eq(idColumn).text('').append(purchaseLink);
            $('td', row).eq(3).text('$' + $('td', row).eq(3).text());
            $('td', row).eq(9).text('$' + $('td', row).eq(9).text());

            var cellData = $('td', row).eq(10).text();
            createNote(noteColumn, false, purchaseId, cellData, row);
        },
