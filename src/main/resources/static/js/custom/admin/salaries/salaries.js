...
$(document).ready(function () {
    dataTable = $('#salaries').DataTable({
        ...
        "scrollCollapse": true,
        "ajax": {
            "url": context + "commissions/salary/get.json",
        ...
        "orderFixed": [[12, "asc"], [0, "asc"], [5, "desc"]],
        "aoColumns": [
            {"mData": "affiliate"},
            {"mData": "vendor"},
            {"mData": "policy"},
        ...
        rowGroup: {
            ...
            endRender: function (rows, group) {
                ...
                return $('<tr />')
                    .append('<td colspan="8">Total ('+ group +'):</td>')
                    .append('<td colspan="1">' + salary + '</td>')
                    .append('<td colspan="1"><input class="payAllInput" type="checkbox" name="payAll" uid ="' + uid + '"' + (isSelected ? ' checked' : '') + ' data-group-name="' + groupName + '"' + '/></td>')
                    .append('<td colspan="2"/>');
            },
        },
        "createdRow": function (row, data, index) {
            ...
            if(!isNaN(rowId)) {
                if(rowId < 0) {
                    ...
                    viewLink = '<a id="' + (-rowId) + '" class="text-center" style="text-decoration:none;"'
                        + ' href="' + context + 'commissions/salary/balance/' + data.affiliateId + '">'
                        + '<span class="glyphicon glyphicon-chevron-right"></span></a>';
                } else {
                    //Link to delete salary correction
                    viewLink = '<a id="sc' + rowId + '" class="text-center" style="text-decoration:none;"'
                        + ' onclick="deleteSalaryCorrection(' +
                        + rowId + ', \'' + $('td', row).eq(affiliateColumn).text() + '\''
                        + ', \'' + $('td', row).eq(salaryColumn).text() + '\''
                ...
                viewLink = '<a id="' + rowId + '" class="text-center ' + disabled + '" style="text-decoration:none;"'
                    + ' href="' + context + 'commissions/purchase/salary/' + rowId + '">'
                    +'<span class="glyphicon glyphicon-chevron-right"></span></a>';
            }
            ...
            var expectedSalaryStr = expectedSalary;
            if (expectedSalary !== '') {
                expectedSalaryStr = expectedSalary < 0 ? ('-$' + (-expectedSalary).toFixed(2)) : ('$' + expectedSalary);
            }
            ...
            var salaryInput = '<input class="salaryInput form-control input-sm dollarInput" type="text" style="width: 100px;" name="salary"'
                ...
                + ' value="' + salary + '"' + ' expectedSalary="' + expectedSalary + '"'
                + ' uid="' + rowId + '" affiliateId="' + data.affiliateId + '"' + disabledSecure + '/>';
            ...
            $('td', row).eq(expectedSalaryColumn).text("").append(expectedSalaryStr);
            $('td', row).eq(receivedCommissionColumn).text("").append(receivedCommission);
    ...
    $('#salaries').on('click', '.payInput', function () {
        ...
        var salaryInput = $(this).parent().parent().find('.salaryInput');
        if (!isChecked){
            payAll[0].checked = false;
        } else {
            var checked = true;
            if(salaryInput.val() <= 0) {
                salaryInput.val(salaryInput.attr('expectedSalary'));
                salaryInput.blur();
            }
        ...
        fillUpdatedFields($(this).attr("uid"), $(this).attr("affiliateId"),  salaryInput.val(), this.name, this.checked);
    });
    ...
    $("#reset").on("click", function () {
        ...
        clearSelectizeInput('#affiliates-correction');
        ...
        $("#vendorCommissionFrom").val(null);
        $("#vendorCommissionTo").val(null);
    ...
    selectizeAffiliatesCorrectionInit();
...
function deleteSalaryCorrection(id, affiliate, amount, date) {
    if(confirm('Delete salary correction for ' + affiliate + ' with the amount of $' + amount + ' on ' + date + '?')) {
        $.post(context + 'commissions/salary/delete/', { id : id }, function () {
            dataTable.draw();
        });
    }
}