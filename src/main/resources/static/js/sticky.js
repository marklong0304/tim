...
(function( $, window ) {
  ...
  scrollHandler = function( e ) {
    ...
    stickytable.refresh( { top: $scrollContainer.scrollTop() + 80,
                           left: scBar.scrollLeft() } );
  },
  ...
  cloneCells = function( $cells, columnCount, maxRows ) {
    ...
    for ( var i = 0; i < $cells.length; i++ ) {
      ...
      if ( i % columnCount === columnCount - 1 ) {
        var attrs = $(td.parentElement).prop("attributes");
        var tr = '<tr ';
        $.each(attrs, function() {
          tr += this.name + '="'+this.value+'" ';
        });
        tr += '>' + cells.join('') + '</tr>';
        rows.push( tr);
  ...
  StickyTable.prototype = {
    ...
    stick : function() {
      ...
      scBar.on( 'scroll', this, scrollHandler );
    ...
    unstick : function() {
      ...
      scBar.off( 'scroll', scrollHandler );
    ...
    refresh : function( offset ) {
      ...
      if ( offset.top >= tableOffSet.top + this.$stickyTableHeader.height()  && headerHidden ) {
        this.$stickyTableHeader.show();
        $('.sticky-header .alt-provname, .alt-plan-name').ellipsis({setTitle:'onEllipsis', live: false});
      }

      this.$stickyTableHeader.css( 'left', (0 * -1) + rawOffset.left );
      ...
      if ( tableOffSet.left < 0 && columnHidden ) {
        this.$stickyTableColumn.show();
      }

      this.$stickyTableColumn.css( 'top', (offset.top * -1) + rawOffset.top + 80);
    ...
    updateCellCount: function () {
      this.cellCount = this.$table.find('tbody tr:eq(0) td:not(.hidden-col)').length;
    },

    redrawHeight: function () {
      var stickyCells = $(".sticky-column>tbody>tr");
      var firstColCells = $(".col-0");
      var offset = 3;
      stickyCells.each(function (key, val) {
        $(val).height($(firstColCells[key + offset]).outerHeight());
      });
    }
  };
