...
jQuery.fn.fakeFloat = function(options, callback)
{
	...
	jQuery(this).each(function()
	{
		...
		{
			jQuery(this).animate({"left": ((settings.margin) + elemWidth)*i + (settings.offset) + 'px'}, settings.speed, settings.easing);
		}
		else
		{
				jQuery(this).animate({"right": ((settings.margin) + elemWidth)*i + (settings.offset) + 'px'}, settings.speed, settings.easing);
		}
		i++;
	});
	...
	if(settings.shiftOnly === false)
	{
		...
		scroller.scroll = function()
		{
			...
			{			
				...
				jQuery(scroller.shiftingElement).css("padding-left", jQuery(scroller.shiftingElement).width()+"px");
				//jQuery(scroller.shiftingElement).animate({"opacity": 0}, 0);
				jQuery(scroller.shiftingElement).animate({"padding-left": 0}, 600, "easeInOutExpo");
